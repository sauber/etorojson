...
# Run docker image
etorojson="docker run --rm etorojson"

# Verify output from Discover People
#
trap "echo discover failed; exit 1" ERR
buf=$($etorojson discover "dailyddmin=-25&gainmin=0&hasavatar=true&maxmonthlyriskscoremax=6&maxmonthlyriskscoremin=1&pagesize=20&popularinvestor=true&sort=-copiers")
...
buf=$($etorojson portfolio jeppekirkbonde 2988943)
...
buf=$($etorojson stats jeppekirkbonde 2988943)
...
buf=$($etorojson chart jeppekirkbonde 2988943)
